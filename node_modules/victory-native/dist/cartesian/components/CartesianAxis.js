...
const CartesianAxis = ({ tickCount, labelPosition, labelOffset, axisSide, lineColor, lineWidth, labelColor, formatYLabel, formatXLabel, yScale, xScale, font, isNumericalData = false, ix, }) => {
    var _a;
    const axisConfiguration = (0, react_1.useMemo)(() => {
        return {
            ...
            frameLineWidth: typeof lineWidth === "number" ? lineWidth : lineWidth.frame,
        };
    }, [
        tickCount,
        labelOffset,
    ...
    const { xTicks, yTicks, xAxisPosition, yAxisPosition, xLabelPosition, yLabelPosition, xLabelOffset, yLabelOffset, gridLineColor, frameLineColor, gridLineWidth, frameLineWidth, } = axisConfiguration;
    ...
    const yAxisNodes = yScale.ticks(yTicks).map((tick) => {
        ...
        <react_native_skia_1.Line p1={(0, react_native_skia_1.vec)(xScale(x1), yScale(tick))} p2={(0, react_native_skia_1.vec)(xScale(x2), yScale(tick))} color={gridLineColor} strokeWidth={gridLineWidth}/>
        {font
    ...
    const xAxisNodes = xScale.ticks(xTicks).map((tick) => {
        ...
        <react_native_skia_1.Line p1={(0, react_native_skia_1.vec)(xScale(tick), yScale(y2))} p2={(0, react_native_skia_1.vec)(xScale(tick), yScale(y1))} color={gridLineColor} strokeWidth={gridLineWidth}/>
    ...
    return (<>
      ...
      <react_native_skia_1.Path path={boundingFrame} strokeWidth={frameLineWidth} style="stroke" color={frameLineColor}/>
    </>);
};
exports.CartesianAxis = CartesianAxis;
exports.CartesianAxis.defaultProps = {
    lineColor: "hsla(0, 0%, 0%, 0.25)",
    lineWidth: {
        grid: react_native_1.StyleSheet.hairlineWidth,
        frame: react_native_1.StyleSheet.hairlineWidth,
    },
    tickCount: 5,
    labelOffset: { x: 2, y: 4 },
